...
const GENERATE_TEMPLATE_PROMPT = `아래 릴스 대본을 분석해서 단계별 스크립트 코치 자료를 만들어줘.
...
4단계: 단계별 코치 생성
대본을 섹션 단위로 분해해 — 훅 / 본문1 / 본문2 / ... / CTA.
섹션 수는 대본 구조에 맞게 자유롭게 결정해.
각 섹션마다 아래 세 가지를 작성해:
- why: 이 섹션의 문장 구조가 시청자에게 주는 심리적 효과, 왜 이 순서/방식으로 써야 하는지 이유 (2~3문장)
- how: 내 상품으로 이 섹션을 쓸 때의 구체적 패턴과 작성 방법 (패턴 예시 포함, 2~3문장)
- example: 레퍼런스 대본에서 이 섹션에 해당하는 실제 문장 그대로 (원문 그대로, 수정 금지)
...
반드시 JSON만 반환:
{
  ...
  "steps": [
    {
      "section": "훅",
      "why": "이 섹션의 심리적 효과와 왜 이 순서여야 하는지 이유",
      "how": "내 상품으로 이 섹션을 쓰는 구체적 방법과 패턴",
      "example": "레퍼런스 대본에서 이 섹션의 실제 문장 그대로"
    },
    {
      "section": "본문1",
      "why": "...",
