...
4. 여성/남성 타겟에 따라 분석 관점을 다르게 적용해
5. 위 22개 유형에 해당하지 않는 새로운 후킹 패턴 발견 시 새 유형명 자동 생성하고 isNewType: true 표시

여성 분석 관점: 감성/공감/변신욕구/군중심리/사회적인정
남성 분석 관점: 기능/근거/수치/인지부조화/가성비/코디고민해결
...
예시4 - 룰루레몬 대신 레깅스:
"여러분! 저는 룰루레몬, 알로 못 입어요! 사실 안입어요" → 태그:후킹 / 룰루레몬/알로는 MZ여성들이 선망하는 고가 요가복 브랜드. 이 브랜드를 못 입는다는 예측오류 발생 → 왜 안입지? 궁금증 유발. 유형4 브랜드반전형.
...
"궁뎅이 눌림 아예 없어요 / 열심히 만든 궁댕이 눌려버리면 화나" → 태그:심리 / 일반적으로 이 상품 착용 시 발생하는 문제 공유. 운동하는 사람의 공감대 자극.
...
"여름에 긴바지 덥지 않냐고?" → 태그:심리 / 시청자가 당연히 가질 반론을 먼저 선제적으로 꺼냄. 여성 릴스와 달리 감성이 아닌 논리로 설득하는 남성 특화 기술.
"퀄리티 좋은 드라이 나일론 원단으로 제작해서 통풍 잘 돼서 시원하고" → 태그:본문 / 반론에 대한 기능적 근거 제시. 소재명/기능을 구체적으로 설명하는 남성 설득 방식.
"다리에 땀 차도 땀이 안붙어서 나처럼 땀 많은 사람도" → 태그:심리 / 보통 긴바지를 여름에 입었을 때 생기는 문제 걱정 해결. 나처럼이라는 표현으로 공감대 형성.
"내가 코디를 못한다? 걱정마 누가 입어도 무조건 기본 이상을 하니까" → 태그:본문 / 남성 핵심 고민(코디 못함) 직접 공략. 걱정마라는 단호한 확신형 어투. 남성 특화.
"이 바지 한 장 사면 19,800원인데 두 장 사면 추가 할인까지 더해서 34,800원" → 태그:본문 / 가격을 수치로 직접 비교. 더 큰 할인으로 구매 안하면 손해본다는 심리 자극.
"화이트는 청량하게 블랙은 멋있게 두 컬러 모두 소장해주면 돼" → 태그:본문 / 컬러마다 다른 분위기 소개로 두 개 구매해야 하는 이유 자극. 똑같은 제품 두 개가 아닌 다른 분위기 상품을 산다는 착각 유발.
"내가 여름에도 긴바지 입는 이유? 이거 입어보면 바로 알 거야" → 태그:심리 / 경험에 빗대어 신뢰도 상승. 바로 알 거야라는 자신감 있는 표현. 남성 특화 도전적 어투.

=== 여성/남성 분석 차이 기준 ===
여성: 감성/공감/변신욕구, 예쁘죠? 강추! 어투, 외모변신/사회적인정/군중심리
남성: 기능/근거/수치/인지부조화, 걱정마/바로알거야 확신형 어투, 코디못함/땀/가성비/논리적설득

=== AI 자동 유형 생성 규칙 ===
위 22개 유형에 해당하지 않는 새로운 후킹 패턴 발견 시:
- hookFormulaType에 새로운 유형명 자동 생성
...
function buildWordContextString(wordContexts) {
  if (!wordContexts || wordContexts.length === 0) return '';
  const lines = ['등장하는 브랜드/아이템 맥락 정보:'];
  wordContexts.forEach(({ word, contextType, customDesc }) => {
    const label = customDesc || CONTEXT_TYPE_LABEL[contextType] || contextType;
    lines.push(`- ${word}: ${label}`);
  });
  lines.push('이 맥락을 반영해서 후킹 구조와 심리 효과를 분석해줘.');
  return lines.join('\n');
}
...
exports.analyzeScript = onCall(
  ...
  async (request) => {
    ...
    const { text, shopType, wordContexts } = request.data;

    if (!text || typeof text !== 'string' || text.trim().length === 0) {
      throw new HttpsError('invalid-argument', '분석할 대본 텍스트를 입력해주세요.');
    }
    if (!shopType || !SHOP_TYPE_LABEL[shopType]) {
      throw new HttpsError('invalid-argument', '올바른 쇼핑몰 타입을 선택해주세요.');
    }
    ...
    const hasContext = Array.isArray(wordContexts) && wordContexts.length > 0;
    const contextStr = hasContext ? buildWordContextString(wordContexts) : '';

    const userContent = hasContext
      ? `쇼핑몰 타입: ${SHOP_TYPE_LABEL[shopType]}\n\n${contextStr}\n\n대본:\n${text.trim()}`
      : `쇼핑몰 타입: ${SHOP_TYPE_LABEL[shopType]}\n\n대본:\n${text.trim()}`;
...
exports.generateScript = onCall(
  ...
  async (request) => {
    ...
    const { productName, features, analysis, shopType } = request.data;
    ...
    try {
      const message = await client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `쇼핑몰 타입: ${SHOP_TYPE_LABEL[shopType]}
