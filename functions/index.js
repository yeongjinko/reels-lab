...
- 첫 번째 질문(주제 묻기): 일반적인 상품/주제 예시 2~3개 (예: "슬랙스", "퍼스널 컬러 진단", "다이어트 식단")
- 두 번째 질문부터: 반드시 사용자가 답한 주제/상품을 examples에 자연스럽게 녹여서 작성
  예) 사용자 주제가 "슬랙스"라면: "제가 슬랙스 직접 입어봤는데 허리가 편해요" (O) / "와이드 슬랙스를 직접 입어봤어요" (X, 레퍼런스 느낌)
- 레퍼런스 영상의 브랜드명, 상품명, 표현 방식 절대 사용 금지

=== suitableFor 작성 규칙 ===

첫 번째 질문(지금까지 대화 없음)인 경우에만:
suitableFor에 이 후킹 유형/공감 포인트가 적합한 상황 3가지를 구체적으로 작성.
(예: "비슷한 제품보다 가성비 좋은 걸 파는 사람", "고객이 특정 문제를 겪고 있고 내가 해결책을 아는 사람")
이후 질문에서는 suitableFor: []로 반환.
...
레퍼런스 대본을 아래 3단계 순서로 분석해서 템플릿을 만들어줘.

=== 템플릿 생성 3단계 ===

1단계: 전체 대본 스캔 — 사람마다 달라지는 요소 목록 추출
아래 카테고리 중 이 대본에 해당하는 것을 모두 찾아:
- 상품명/서비스명
- 타겟 (특정 사람 언급)
- 검증/비교 아이템
- 핵심 특징/장점
...
2단계: 추출된 요소들을 태그명으로 정의
각 요소에 태그명 부여 (역할을 나타내는 2~6자)
예) 상품명 → [상품명], 기본 흰티 → [검증아이템], 룰루레몬 → [검증아이템]

3단계: 전체 대본에서 해당 단어가 나오는 곳 전부 태그로 교체
- 같은 요소면 반드시 같은 태그명 사용
- 대본 전체에 일관되게 적용
- 문장당 태그 최대 1~2개 유지
...
=== 태그 치환 추가 규칙 ===
1. 감탄사·접속사·리액션·구조 자체가 핵심인 문장 → 태그 없이 그대로
   예) "드디어 찾았어요", "이게 뭐냐구요?", "심지어", "잖아" → 수정 금지
2. 태그명 예시: 상품명, 타겟, 핵심특징, 문제점, 장점, 가격대, CTA키워드, 혜택, 검증아이템, 평가결과

=== 조사 힌트 규칙 ===
태그 바로 뒤에 한국어 조사가 오는 경우: [태그명:조사힌트] 형식.
지원 힌트: 이/가, 을/를, 은/는, 과/와, 으로/로
예) "[상품명:이/가] 편해요" → 팬츠가 편해요 (받침 없음) / 슬랙스가 편해요
예) "[타겟:을/를] 위해" → 임산부를 위해 (받침 없음) / 남성들을 위해
예) "[핵심특징:은/는] 살려줘요" → 탄력은 살려줘요 (받침 있음) / 광택은 살려줘요
주의: 조사가 바로 뒤에 없으면 그냥 [태그명]

=== tags 배열 규칙 ===
템플릿에 나오는 모든 고유 태그를 한 번씩 포함. (조사 힌트 제외한 순수 태그명 기준)
- tag: 순수 태그명
- description: 이 자리에 뭘 써야 하는지 (15자 이내)
- example: 구체적 예시
- occurrences: 이 태그가 템플릿에 등장하는 횟수

=== sentences 배열 규칙 ===
template를 \\n으로 split한 줄 수와 반드시 동일한 개수.
- text: 레퍼런스 원문 그대로
- role: 이 줄의 역할 (예: 후킹 문장, 공감대 형성, 가격 반전)
- effect: 시청자 심리 효과 (1~2문장)

=== suitableFor ===
이 후킹 구조가 잘 맞는 상황 3가지.
...
출력:
{
  ...
  "isNewType": false,
  "empathyPoint": "고가 브랜드를 안 입는다는 반전 고백으로 예측오류를 만들어 시청자가 멈추게 한다.",
  "suitableFor": ["비슷한 제품보다 가성비 좋은 걸 파는 사람", "당연히 쓰는 브랜드 대신 내 것을 소개하는 사람", "인지도는 낮지만 품질 좋은 상품을 판매하는 사람"],
  "template": "여러분! 저는 [검증아이템], [검증아이템2] 못 입어요! 사실 안입어요\n대신 이거 입어요 / 이게 뭐냐구요?\n[검증아이템] 반의 반값으로 살 수 있어요!",
  "tags": [
    { "tag": "검증아이템", "description": "비교 대상 유명 브랜드", "example": "룰루레몬", "occurrences": 2 },
    { "tag": "검증아이템2", "description": "두 번째 비교 브랜드", "example": "알로", "occurrences": 1 }
  ],
  "sentences": [
    { "text": "여러분! 저는 룰루레몬, 알로 못 입어요! 사실 안입어요", "role": "후킹 문장", "effect": "선망 브랜드를 안 입는다는 반전 고백으로 예측오류를 만든다." },
    { "text": "대신 이거 입어요 / 이게 뭐냐구요?", "role": "호기심 증폭", "effect": "대안 제시 + 궁금증 극대화." },
    { "text": "알로 반의 반값으로 살 수 있어요!", "role": "가격 반전", "effect": "고가 브랜드와 비교해 충격적 가성비 강조." }
  ]
}

조사 힌트 예시: "[상품명:이/가] 정말 편해요" → 레깅스가 정말 편해요
...
이 답변들을 기반으로 레퍼런스의 구조와 흐름은 유지하되,
사용자만의 완전히 새로운 대본을 작성해줘.

핵심 규칙:
1. 레퍼런스 문장을 절대 그대로 복사하지 말 것
2. 사용자 답변의 내용을 반드시 반영할 것 (답변이 비어있으면 해당 문장의 심리적 역할에 맞게 창의적으로 작성)
3. 자연스럽고 구어체로 작성 (릴스/숏폼 말투)
4. 각 문장은 레퍼런스에서 배운 심리적 역할을 수행해야 함
5. 전체 대본이 하나의 자연스러운 흐름으로 연결되게
6. 카피가 아닌 영감 받은 사용자만의 대본으로
...
exports.generateQuestions = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const userTopic = history.length > 0 ? history[0].answer.trim() : '';
      ...
      let userContent = `레퍼런스 후킹 유형: ${hookType}
레퍼런스 공감 포인트: ${empathyPoint || ''}
${historyText ? `\n지금까지 대화:\n${historyText}` : ''}${nextInstruction}`;

      if (userTopic) {
        userContent += `\n\n[examples 필수 규칙] 사용자 주제/상품: "${userTopic}" — 이번 examples는 반드시 이 주제가 자연스럽게 들어간 표현으로 작성. 레퍼런스 브랜드명/상품명 사용 금지.`;
      }
...
exports.generateFinalScript = onCall(
  ...
  async (request) => {
    ...
    const { steps, hookType } = request.data;
    if (!steps || !Array.isArray(steps) || steps.length === 0) {
      throw new HttpsError('invalid-argument', '스텝 정보가 필요합니다.');
    }

    const client = new Anthropic({ apiKey: anthropicApiKey.value() });
    const stepsStr = steps.map((s, i) =>
      `STEP ${i + 1}:\n레퍼런스 문장: "${s.sentence}"\n역할: ${s.role}\n심리 효과: ${s.effect}\n질문: ${s.question}\n사용자 답변: ${s.userInput || '(없음)'}`
    ).join('\n\n');

    try {
      console.log('generateFinalScript START — steps:', steps.length, '| hookType:', hookType);
      const t0 = Date.now();
      const message = await client.messages.create({
        model: MODEL,
        max_tokens: 2048,
        system: GENERATE_FINAL_SCRIPT_PROMPT,
        messages: [{
          role: 'user',
          content: `후킹 유형: ${hookType || ''}\n\n레퍼런스 문장별 분석 및 사용자 답변:\n${stepsStr}`,
        }],
      });
      const rawText = message.content[0].text;
      console.log('generateFinalScript API done —', Date.now() - t0, 'ms');
      const result = parseJsonFromText(rawText);
      return { success: true, data: result };
    } catch (e) {
      console.error('generateFinalScript error:', e);
      if (e instanceof HttpsError) throw e;
      throw new HttpsError('internal', '대본 생성 중 오류가 발생했습니다.');
    }
  }
);
